...
export class Chunk {
  ...
  rebuildMesh(
    ...
    const blockLights: MeshNumberBuffer = [];
    ...
    const skyExposure = createChunkSkyExposure((localX, y, localZ) => {
      const worldX = ox + localX;
      const worldZ = oz + localZ;
      return world.isRenderableSolid?.(worldX, y, worldZ) ?? world.isSolid(worldX, y, worldZ);
    });

    this.buildXFaces(world, skyExposure, ox, oz, positions, normals, colors, blockLights, uvs, textureTiles, indices);
    this.buildYFaces(world, skyExposure, ox, oz, positions, normals, colors, blockLights, uvs, textureTiles, indices);
    this.buildZFaces(world, skyExposure, ox, oz, positions, normals, colors, blockLights, uvs, textureTiles, indices);

    const geometry = new THREE.BufferGeometry();
    geometry.setAttribute("position", new THREE.Float32BufferAttribute(positions, 3));
    geometry.setAttribute("normal", new THREE.Float32BufferAttribute(normals, 3));
    geometry.setAttribute("color", new THREE.Float32BufferAttribute(colors, 3));
    geometry.setAttribute("blockLight", new THREE.Float32BufferAttribute(blockLights, 1));
  ...
  applyMeshData(
    ...
    geometry.setAttribute("blockLight", new THREE.BufferAttribute(meshData.blockLights, 1));
  ...
  buildXFaces(
    ...
    for (let x = 0; x < CHUNK_SIZE; x += 1) {
      emitGreedyFaces(
        WORLD_HEIGHT,
        CHUNK_SIZE,
        (y, z) => exposedBlock(this, world, skyExposure, x, y, z, x + 1, y, z, ox + x + 1, y, oz + z, 1, 0, 0),
  ...
  buildYFaces(
    ...
    for (let y = 0; y < WORLD_HEIGHT; y += 1) {
      emitGreedyFaces(
        CHUNK_SIZE,
        CHUNK_SIZE,
        (x, z) => exposedBlock(this, world, skyExposure, x, y, z, x, y + 1, z, ox + x, y + 1, oz + z, 0, 1, 0),
  ...
  buildZFaces(
    ...
    for (let z = 0; z < CHUNK_SIZE; z += 1) {
      emitGreedyFaces(
        CHUNK_SIZE,
        WORLD_HEIGHT,
        (x, y) => exposedBlock(this, world, skyExposure, x, y, z, x, y, z + 1, ox + x, y, oz + z + 1, 0, 0, 1),
...
function exposedBlock(
  ...
  const worldX = neighborWorldX - _normalX;
  const worldY = neighborWorldY - _normalY;
  const worldZ = neighborWorldZ - _normalZ;
