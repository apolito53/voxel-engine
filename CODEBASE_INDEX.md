...
- Change thrown object behavior, core/debris/rubble collision, debris lifetime, debris grid size, quality-scaled visible debris counts, stable rubble material units, instanced debris rendering, rubble cover proxies, object budget, despawn controls, or impact damage: `src/blockFragments.ts`, `src/physics.ts`, `src/physicsInstancing.ts`, `src/rubble.ts`, per-quality defaults in `src/qualityPresets.ts`, persistence bounds in `src/physicsBudget.ts`, `VoxelWorld.damageBlock` in `src/world.ts`, plus selected Physics Core use, `KeyX`, `PhysicsToyCollider`, `RubbleField`, `clearPhysicsCores`, `clearToys`, and `handlePhysicsImpact` in `src/main.ts`.
