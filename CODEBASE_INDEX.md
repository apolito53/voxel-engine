...
- Persistent destructible rubble cover proxies, support/fall/merge rules, and dense terrain-block promotion: `src/rubble.ts`
...
- Rubble piles are the persistent gameplay representation of settled debris. `RubbleField` keeps visual piles, cover raycasts, health, core collision, support checks, fall/merge behavior, and promotion into the generated `Rubble` block together; do not reintroduce permanent per-shard gameplay collision when tuning cover behavior.
