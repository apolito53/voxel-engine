...
- In-memory partial-block terrain carving, one-health core carve constants, and the shared faceted custom-cell mesh owner that renders chipped blocks while collision stays voxel-solid for now: `src/partialBlocks.ts`
...
- Partial-terrain carving bolt-on: `src/partialBlocks.ts`, carve ownership in `src/world.ts`, partial render masks in `src/chunkProtocol.ts`/`src/chunkWorker.ts`, normal-cube suppression in `src/chunk.ts`, and `handlePhysicsImpact`/`PartialBlockMeshField` wiring in `src/main.ts`. It owns visual custom-cell geometry and terrain health carving, but first-pass collision remains full voxel via `VoxelWorld.isSolid`; save persistence and non-cuboid collision are future deliberate steps.
...
- Change thrown object behavior, partial terrain carving, parked impact craters, core/debris/rubble collision, debris lifetime, debris grid size, debris shapes, quality-scaled visible debris counts, stable rubble material units, settling-region behavior, Rapier debris bodies/colliders, instanced debris rendering, rubble cover proxies, walkable rubble support, object budget, despawn controls, damage indicators, or impact damage: `src/blockFragments.ts`, `src/debrisShapes.ts`, `src/partialBlocks.ts`, `src/impactCraterField.ts`, `src/rigidDebris.ts`, `src/debrisSettler.ts`, `src/fragmentRubble.ts`, `src/physics.ts`, `src/physicsCoreSettings.ts`, `src/physicsInstancing.ts`, `src/rubble.ts`, `src/collision.ts`, `src/damageIndicators.ts`, per-quality defaults in `src/qualityPresets.ts`, persistence bounds in `src/physicsBudget.ts`, `VoxelWorld.damageBlock`/`VoxelWorld.carveBlock` in `src/world.ts`, plus selected Physics Core use, `KeyX`, `PhysicsToyCollider`, `RigidDebrisSimulation`, `RubbleField`, `PartialBlockMeshField`, `clearPhysicsCores`, `clearToys`, and `handlePhysicsImpact` in `src/main.ts`.
...
- Physics-core size and velocity sliders affect newly thrown player and Nova cores. Core-vs-terrain contact uses a swept sphere path so tiny fast cores hit the first block along the segment instead of tunneling into the block behind it; partial-block carve work should preserve that first-contact ordering.
...
- Partial-block terrain carving is in-memory first-pass terrain state. `VoxelWorld.carveBlock` keeps chipped cells solid for collision and raycast through `isSolid`, but `isRenderableSolid` makes chunk meshing skip those cube cells while `PartialBlockMeshField` draws the custom faceted volume. Partial render masks must travel with worker mesh requests or the worker will resurrect full cubes over carved cells. Partial cells are not saved to IndexedDB yet; leaving the active world clears them.
