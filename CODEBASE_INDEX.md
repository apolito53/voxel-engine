...
- The app code owns chunks, terrain generation, voxel meshing, player movement, collision, ray picking, held-item action routing, HUD, minimap, admin/test-lab tooling, impact damage, crater/scar visuals, debris lifetime/bake-out, rubble cover proxies, and simple physics toys/cores.
...
- App bootstrap, render loop, input glue, world lifecycle orchestration, terrain/rubble target selection, impact crater wiring, damage indicator wiring, and WebGL runtime teardown: `src/main.ts`
...
- Capped static faceted crater/scar mesh stamped from physics-core terrain impact points; destroyed-block hits stamp surviving exposed faces and hosted scars are removed when their block breaks; visual-only for now: `src/impactCraterField.ts`
...
- Impact-scar bolt-on: `src/impactCraterField.ts` plus the `handlePhysicsImpact` stamp call in `src/main.ts`. It should stay visual-only until crater gameplay exists: it depends on block id, impact point, block cell, normal, and speed, but it should not own terrain damage, rubble material, or collision.
...
- Change thrown object behavior, impact craters, core/debris/rubble collision, debris lifetime, debris grid size, debris shapes, quality-scaled visible debris counts, stable rubble material units, settling-region behavior, Rapier debris bodies/colliders, instanced debris rendering, rubble cover proxies, walkable rubble support, object budget, despawn controls, damage indicators, or impact damage: `src/blockFragments.ts`, `src/debrisShapes.ts`, `src/impactCraterField.ts`, `src/rigidDebris.ts`, `src/debrisSettler.ts`, `src/fragmentRubble.ts`, `src/physics.ts`, `src/physicsCoreSettings.ts`, `src/physicsInstancing.ts`, `src/rubble.ts`, `src/collision.ts`, `src/damageIndicators.ts`, per-quality defaults in `src/qualityPresets.ts`, persistence bounds in `src/physicsBudget.ts`, `VoxelWorld.damageBlock` in `src/world.ts`, plus selected Physics Core use, `KeyX`, `PhysicsToyCollider`, `RigidDebrisSimulation`, `RubbleField`, `ImpactCraterField`, `clearPhysicsCores`, `clearToys`, and `handlePhysicsImpact` in `src/main.ts`.
...
- `ImpactCraterField` is a visual-only first pass: it caps static stamps, moves destroyed-block hits onto surviving exposed faces, removes scars hosted by destroyed blocks, and clears through `Despawn All Objects`/world teardown. It must not become terrain collision, rubble material, or save data until crater gameplay is designed deliberately.
- `Despawn All Objects` uses the full cleanup path and releases high-water instanced debris batches plus impact craters, while `X` removes only thrown physics cores and preserves fragment/rubble/crater experiments.
