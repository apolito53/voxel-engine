...
- Vite browser app using native ES modules.
...
- Chunk dimensions, voxel storage, top-column cache, mesh rebuilds: `src/chunk.js`
- Chunk ownership, terrain generation, streaming, reads/writes: `src/world.js`
...
3. `VoxelWorld.ensureChunksAround` creates initial chunks; generated chunks use `fbm2` terrain noise.
4. `Chunk.rebuildMesh` builds vertex-colored visible voxel faces for dirty chunks.
5. Each animation frame updates player motion, chunk streaming, dirty mesh rebuilds, physics toys, HUD/debug text, minimap, and final render.
6. Block edits go through `voxelRaycast` plus `VoxelWorld.setBlock`, then neighboring chunks are marked dirty when edge blocks change.

## Common Change Targets

- Add or recolor blocks: update `src/blocks.js`; inspect mesh color use in `src/chunk.js`.
- Tune terrain: update `VoxelWorld.generateChunk` in `src/world.js`; terrain noise lives in `src/math.js`.
