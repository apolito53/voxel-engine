...
- Capped static faceted crater/scar mesh stamped from physics-core terrain impact points; visual-only for now: `src/impactCraterField.ts`
...
- `ImpactCraterField` is a visual-only first pass: it caps static stamps and clears through `Despawn All Objects`/world teardown, but it must not become terrain collision, rubble material, or save data until crater gameplay is designed deliberately.
