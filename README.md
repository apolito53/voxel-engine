...
- Local browser test runs can use `globalThis.__VOXEL_VISUAL_TEST__.listScenarios()`, `scenarioSnapshot("debris-pressure")`, and `recordScenario("debris-grounding")` while `npm.cmd run debug:logs` is listening; the browser records the game canvas as WebM, samples review frames, and posts the run to `logs/visual-runs/YYYY-MM-DD/...` with `recording.webm`, extracted video frames when local `ffmpeg` is available, `manifest.json`, and `review.html`. Scenario snapshots and recording manifests include compact runtime pressure/hitch receipts so videos are not the only evidence. Current scripted shots include `preview-parity`, `debris-pressure`, `debris-grounding`, `hitscan-tunnel`, `builder-fixture`, `wall-range`, and `free-roam`; `recordPilotPlay("wall-range")` remains as a compatibility shortcut.
